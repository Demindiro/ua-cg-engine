...
constexpr void icosahedron(render::Face faces[20]) {
	for (unsigned int i = 0; i < 5; i++) {
...
render::TriangleFigure icosahedron(const FigureConfiguration &conf);

void fractal_icosahedron(const FigureConfiguration &conf, std::vector<Line3D> &lines);

render::TriangleFigure fractal_icosahedron(const FigureConfiguration &conf);
