...
UnexpectedCharacter::UnexpectedCharacter(
	...
	: ParseException(), character(character_init), position(position_init),
	  message("") {
...
UnexpectedCharacter::UnexpectedCharacter(
	...
	: ParseException(), character(original.character),
	  position(original.position), message(original.message) {
	// Does nothing...
}
...
DuplicateEntry::DuplicateEntry(const DuplicateEntry &original) throw()
	: ParseException(), section(original.section), key(original.key),
	  message(original.message) {
	// Does nothing...
}
...
NonexistentEntry::NonexistentEntry(const std::string &section_name_init,
	...
	: std::exception(), section_name(section_name_init),
	  entry_name(entry_name_init), message("") {
...
IncompatibleConversion::IncompatibleConversion(
	...
	: std::exception(), section_name(section_name_init),
	  entry_name(entry_name_init), type_name(type_name_init), message("") {
...
IncompatibleConversion::IncompatibleConversion(
	...
	: std::exception(original), section_name(original.section_name),
	  entry_name(original.entry_name), type_name(original.type_name),
	  message(original.message) {
	// Does nothing...
}
...
void assert_chars(std::istream &input_stream, const char *const chars) {
	for (const char *i = chars; *i != '\0'; ++i) {
		if (input_stream.peek() != *i) {
			throw UnexpectedCharacter(input_stream.peek(),
									  input_stream.tellg());
		}
...
Entry::Entry(const std::string &section_name_init,
	...
	: section_name(section_name_init), entry_name(entry_name_init),
	  value_ptr(value_ptr_init) {
	// Does nothing...
}

Entry::Entry(const Entry &original)
	: section_name(original.section_name), entry_name(original.entry_name),
...
DoubleTuple
Entry::as_double_tuple_or_default(const DoubleTuple &def_val) const {
	DoubleTuple value;
